...
export default function WebkameraVerwaltungDetailPage() {
  ...
  const [record, setRecord] = useState<WebkameraVerwaltung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getWebkameraVerwaltung();
  ...
  async function handleUpdate(fields: WebkameraVerwaltung['fields']) {
    if (!record) return;
    await LivingAppsService.updateWebkameraVerwaltungEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteWebkameraVerwaltungEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/webkamera-verwaltung');
  }
