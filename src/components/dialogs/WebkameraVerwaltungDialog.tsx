...
export function WebkameraVerwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: WebkameraVerwaltungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kamera_name": string | null, // Kameraname\n  "kamera_standort": string | null, // Standortbeschreibung\n  "kamera_url": string | null, // Stream-URL\n  "kamera_beschreibung": string | null, // Beschreibung\n  "kamera_status": LookupValue | null, // Status (select one key: "aktiv" | "inaktiv" | "wartung") mapping: aktiv=Aktiv, inaktiv=Inaktiv, wartung=In Wartung\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<WebkameraVerwaltung['fields']>;
      });
      if (gps) {
        setFields(f => ({ ...f, kamera_geo: { lat: gps.latitude, long: gps.longitude, info: geoAddr } as any }));
  ...
  const FIELD_LABELS: Record<string, string> = {"kamera_name": "Kameraname", "kamera_standort": "Standortbeschreibung", "kamera_url": "Stream-URL", "kamera_geo": "Geografischer Standort", "kamera_beschreibung": "Beschreibung", "kamera_status": "Status"};
